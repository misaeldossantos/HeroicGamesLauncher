...
const contextMenu = async () => {
    const recentsMenu = (await getRecentGames({ limited: true })).map(
        (game) => {
            return {
                click: function () {
                    handleProtocol(mainWindow, [
                        `heroic://launch/${game.appName}`
                    ])
                },
                label: game.title
            }
...
if (!gotTheLock) {
    ...
    app.whenReady().then(async () => {
        ...
        const currentConfigStore = configStore.get(
            'settings',
            {}
        ) as AppSettings
        ...
        setTimeout(() => {
            const zoomFactor =
                parseFloat(configStore.get('zoomPercent', '100') as string) /
                100

            mainWindow.webContents.setZoomFactor(
                processZoomForScreen(zoomFactor)
            )
        }, 200)
...
ipcMain.on('lock', () => {
    if (!existsSync(join(heroicGamesConfigPath, 'lock'))) {
        ...
        if (!powerId) {
            logInfo('Preventing machine to sleep', {
                prefix: LogPrefix.Backend
            })
...
ipcMain.on('unlock', () => {
    if (existsSync(join(heroicGamesConfigPath, 'lock'))) {
        ...
        if (powerId) {
            logInfo('Stopping Power Saver Blocker', {
                prefix: LogPrefix.Backend
            })
...
ipcMain.handle(
    ...
    async (event, { appName, launchArguments, runner }): StatusPromise => {
        ...
        if (!powerDisplayId) {
            logInfo('Preventing display from sleep', {
                prefix: LogPrefix.Backend
            })
...
ipcMain.handle('openDialog', async (e, args) => {
    ...
    const { filePaths, canceled } = await showOpenDialog(focusedWindow, args)
    ...
    async (event, { appName, path, runner }) => {
        ...
        switch (runner) {
            ...
            case 'gog':
                ...
                break
            default:
                logError(`Unsupported runner ${runner}`, {
                    prefix: LogPrefix.Backend
                })
                return
        }
