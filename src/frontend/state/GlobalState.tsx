...
export class GlobalState extends PureComponent<Props> {
    state: StateProps = {
        ...
        epic: {
            library: libraryStore.has('library')
                ...
                : [],
            username:
                (configStore.get('userInfo', null) as UserInfo)?.displayName ||
                null
        },
        gog: {
            ...
            username:
                (gogConfigStore.get('userData', null) as any)?.username || null
        },
        ...
        showFavourites: JSON.parse(
            storage.getItem('show_favorites') || 'false'
        ),
        ...
        allTilesInColor:
            (configStore.get('allTilesInColor') as boolean) || false,
        activeController: '',
        ...
        lastChangelogShown: JSON.parse(
            storage.getItem('last_changelog') || 'null'
        )
    }
    ...
    async componentDidMount() {
        ...
        window.api.handleInstallGame(async (e: Event, args: InstallParams) => {
            ...
            if (
                !currentApp ||
                (currentApp && currentApp.status !== 'installing')
            ) {
                const gameInfo = await getGameInfo(appName, runner)
                if (!gameInfo) {
                    return
